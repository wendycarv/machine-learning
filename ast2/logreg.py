...
class LogReg:
    ...
    def sg_update(self, train_example, iteration,
        ...
        # get word (x) and label (y) of datapoint
        self.x = train_example.x
        self.y = train_example.y

        # compute the linear combination of features and weights (z = beta[0] + beta[1:].dot(x))
        z = np.dot(self.beta, self.x)  # this is the weighted sum (dot product)

        # compute the probability p using the sigmoid function
        p = 1 / (1 + np.exp(-z))

        self.beta = self.beta + self.step(iteration)*(self.y-p)*self.x
        #self.beta = self.beta*(1-2*self.step(iteration)*self.mu)+self.step(iteration)*(y-p)*x
        return self.beta

    def find_predictors(self, vocab_list, num_words=10):
        """
        Find the best and worst predictors for a class based on beta coefficients.
        :param vocab_list: List of vocabulary words (features).
        :param num_words: Number of top words to show for each class.
        """

        # sort beta coefficients and grab indices
        sorted_indices = np.argsort(self.beta)

        # print top words for baseball (positive class)
        print(f"\nTop {num_words} predictors for baseball:")
        for i in sorted_indices[-num_words:]:
            print(f"{vocab_list[i]}, {self.beta[i]}")

        # print top words for baseball (negative class)
        print(f"\nTop {num_words} predictors for hockey:")
        for i in sorted_indices[:num_words]:
            print(f"{vocab_list[i]}, {self.beta[i]}")

        # print words with beta value close to zero
        close_to_zero = np.argsort(np.abs(self.beta))[:num_words]
        print(f"\nWorst predictors:")
        for i in close_to_zero:
            print(f"{vocab_list[i]}, {self.beta[i]}")
...
if __name__ == "__main__":
    ...
    # Display best and worst predictors
    # lr.find_predictors(vocab, num_words=10)